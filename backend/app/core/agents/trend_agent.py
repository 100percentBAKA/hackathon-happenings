...
class TrendAnalysis(BaseModel):
    ...
    competitor_analysis: List[str] = Field(
...
class TrendAgent:
    def __init__(self):
        ...
        self.llm = ChatGroq(temperature=0, model_name="llama-3.2-3b-preview")
        self.model = self.llm
        ...
        try:
            ...
            for result in results.get("results", []):
                if "market" in result.get("title", "").lower():
                    ...
                    # Convert competitor analysis strings into dictionaries
                    processed_results["competitor_analysis"].append(
                        {
                            "competitor": result.get("title", "Unknown Competitor"),
                            "strategy": result.get("summary", "No strategy available"),
                        }
                    )

            return processed_results
        except Exception as e:
            print(f"error while processing search results: {str(e)}")
            raise e
    ...
    def _invoke_groq_model(self, prompt: str) -> str:
        """Invoke the ChatGroq model."""
        try:
            response = self.model.invoke(prompt)
            return response.content
        except Exception as e:
            raise Exception(f"Error calling ChatGroq model: {str(e)}")
